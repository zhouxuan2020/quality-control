...
Note we computed aver2_j based using `geno-unrel` data. see \@ref(aver2j).
Results are stored in `summary/ave-r2-by-snp-goodgwas`
...
for(i in 1:length(traits)){
    ...
    file=paste0("unrelated/gwas-good-out/",trait,".out")
...
png(paste0("fig/chisq-by-aver2-gwas-good-binned.png"),
...
png(paste0("fig/chisq-by-aver2-gwas-good-raw.png"),
      width = 50, height = 30, units = "cm", res=600)
  par(mfrow=c(3,5))
...
# plot chisq from good gwas without correcting for PC --------------------------
# based on gwas-mix-0k-noneuro, maf = 0.01
...
png(paste0("fig/chisq-by-aver2-gwas-mix-0k-noneuro-raw.png"),
      width = 50, height = 30, units = "cm", res=600)
  par(mfrow=c(3,5))

for (i in 1:length(traits)){
    trait=traits[i]
    file=paste0("gwas-mix-all-out/",trait,"-mix-pop-gwas-0k-noneuro-maf.01.out")
    dat=vroom(file, col_names=T, delim=";")
    out=dat[complete.cases(dat),]
...
For the bad gwas, we corrected genotypes for PCs and found that the correction does not seem get aver2 to a level equivalent to that from randomly generated SNPs. For the good gwas, we do the same using covariates including geno batch, assessment centre, geographic location, and PCs. This analysis is motivated by the observation that aver2 for the good gwas is significantly higher than what would be expected from randomly generated SNPs.

We add one covariate at a time to see their impact on aver2.
...
## id info ---------------------------------
# note: we noted that among the 100k individuals of the good gwas, variance of 
# of some covariance is zero. so that ldak stop working. So here we can not use
# all ids; but instead use the ids for good gwas.

#id=read.table("unrelated/rand.100000", header=F)
#m=match(id$V1,cov$eid)
#cov=cov[m,]

# coding for geno batch
batches=unique(cov$geno_batch[!is.na(cov$geno_batch)])
for(i in 1:length(batches)){
  batch=batches[i]
  out0=rep(1,dim(cov)[1])
  out0[cov$geno_batch!=batch & !is.na(cov$geno_batch)]=0
  out0[is.na(cov$geno_batch)]=NA
  if(var(out0, na.rm=T)==0) next
  
  if(i==1){out=cbind(cov$eid,cov$eid,out0);
          geno_batch_nm=cbind("eid", "eid", batch)
          } else {out=cbind(out,out0);
          geno_batch_nm=c(geno_batch_nm,batch)}
}
geno_batch=out

write.table(geno_batch,"phen/geno-batch.use", 
            col.names=F, row.names=F,quote=F)
write.table(geno_batch_nm,"phen/geno-batch.use-names", 
            col.names=F, row.names=F,quote=F)

# coding for assessment centre 
centres=unique(cov$centre[!is.na(cov$centre)])
for(i in 1:length(centres)){
  centre=centres[i]
  out0=rep(1,dim(cov)[1])
  out0[cov$centre!=centre & !is.na(cov$centre)]=0
  out0[is.na(cov$centre)]=NA
  if(var(out0, na.rm=T)==0) next
  if(i==1){out=cbind(cov$eid,cov$eid,out0);
  assess_centre_nm=cbind("eid", "eid", centre)
  } else {out=cbind(out,out0);
  assess_centre_nm=c(assess_centre_nm, centre)}
}
assess_centre=out

write.table(assess_centre,"phen/assessment-centre.use", 
            col.names=F, row.names=F,quote=F)
write.table(assess_centre_nm,"phen/assessment-centre.use-names", 
            col.names=F, row.names=F,quote=F)

# other covariates
others=c("sex_gen", "townsend")
for(i in 1:length(others)){
 out=cbind(cov$eid, cov$eid, cov[,others[i]])
 names(out)=c("eid", "eid", others[i])
 write.table(out,paste0("phen/",others[i],".use"), 
            col.names=F, row.names=F,quote=F)
write.table(names(out),paste0("phen/",others[i],".use-names"), 
            col.names=F, row.names=F,quote=F)
}

# geographic locations of birth place
north=phen/other-traits/north-coord-birth-place.raw.pheno
east=phen/other-traits/east-coord-birth-place.raw.pheno
out=phen/birth-place.use

awk 'NR==FNR{a[$1]; b[$1]=$3; next}
     {if ($1 in a) print $1, $2, b[$1], $3}' $north $east > $out
...
mkdir inflation/adjust-geno-good-gwas
...
snps=../gen/snps-unrel-inds.use
bfile=../gen/geno-unrel
cov=../phen/PC-$i.use
out=../inflation/adjust-geno-good-gwas/aver2-lista-PC$i
...
./ldak5.2 --max-threads 7 \
          ...
          --lista $lista \
          --listb $listb
">sh_script/calc-aver2-lista-PC$i
done

# job submission
for i in {1..10} 20 40; do
sbatch -A snpher ../sh_script/calc-aver2-lista-PC$i 
done>../job-records/calc-aver2-adj-geno-lista
...
bfile=../gen/geno-unrel
cov=../phen/PC-$i.use
out=../inflation/adjust-geno-good-gwas/aver2-listb-PC$i
...
./ldak5.2 --max-threads 7 \
          ...
          --lista $listb \
          --listb $lista
">sh_script/calc-aver2-listb-PC$i
done

# job submission
for i in {1..10} 20 40; do
sbatch -A snpher ../sh_script/calc-aver2-listb-PC$i 
done>../job-records/calc-aver2-adj-geno-listb

# merge aver2 of the two lists--------------------------------------------------
dir=inflation/adjust-geno-good-gwas

for i in {1..10} 20 40; do
a=$dir/aver2-lista-PC$i.rjk2.average
b=$dir/aver2-listb-PC$i.rjk2.average
out=$dir/PC-$i.aver2
...
# lista-------------------------------------------------------
for i in {geno-batch,assessment-centre,sex_gen,townsend,birth-place}; do
lista=../inflation/left-snps-unrel-inds-10k
listb=../inflation/right-snps-unrel-inds-10k
id=../unrelated/rand.100000
snps=../gen/snps-unrel-inds.use
bfile=../gen/geno-unrel
cov=../phen/$i.use
out=../inflation/adjust-geno-good-gwas/aver2-lista-$i
...
./ldak5.2 --max-threads 7 \
          ...
          --lista $lista \
          --listb $listb
">sh_script/calc-aver2-lista-$i
done

# job submission
# sex_gen,townsend,birth-place
for i in {geno-batch,assessment-centre}; do
sbatch -A snpher ../sh_script/calc-aver2-lista-$i 
done>../job-records/calc-aver2-adj-geno-lista
...
bfile=../gen/geno-unrel
cov=../phen/$i.use
out=../inflation/adjust-geno-good-gwas/aver2-listb-$i
...
./ldak5.2 --max-threads 7 \
          ...
          --lista $listb \
          --listb $lista
">sh_script/calc-aver2-listb-$i
done

# job submission
#,sex_gen,townsend,birth-place
for i in {geno-batch,assessment-centre}; do
sbatch -A snpher ../sh_script/calc-aver2-listb-$i 
done>../job-records/calc-aver2-adj-geno-listb
...
for i in {geno-batch,assessment-centre,sex_gen,townsend,birth-place}; do
a=$dir/aver2-lista-$i.rjk2.average
b=$dir/aver2-listb-$i.rjk2.average
out=$dir/$i.aver2
awk 'FNR > 1 {print $1, $2}'  $a $b > $out
done
