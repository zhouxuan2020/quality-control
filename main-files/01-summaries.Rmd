...
To compare the proposed method against existing ones, we performed good and bad GWASs for 14 traits using N = 100k. While both sets of GWASs were based on strictly quality-controlled genotype data, bad GWASs used individuals of different ethnicity and hence were confounded by population stratification that are known to cause inflation in test statistics. For the good GWASs, with few exceptions, inflation was not detected by the methods ( Tables \@ref(tab:ldsc-sumher-good) ). In contrast, for bad GWASs significant inflation was evident by all methods ( see Tables \@ref(tab:ldsc-sumher-bad) & \@ref(tab:he-bad) ). The detected inflation for the GWASs is not due to random errors because no inflation was observed for the control GWASs ( see Tables \@ref(tab:ldsc-sumher-control ) & see Tables \@ref(tab:he-control) )
