...
for(i in 1:length(n)){
  for(j in 1:length(m)){
    ...
    start=min(c(dat1.1$V2,dat1.2$V2,dat2.1$V2,dat1.2$V2))
    end=max(c(dat1.1$V2,dat1.2$V2,dat2.1$V2,dat1.2$V2))
      ...
      points(dat1.2$V2, dat2.2$V2,
           cex = 1.5, pch=21, col="white", bg="lightgray", lwd=0.5)
      abline(0, 1, col="darkgray", lwd=1.5, lty=1)
      
      if(i==1 & j==1) {legend("topleft", pch=19, legend=c("maf > .1","maf < .1"), 
             col=c("orange", "lightgray"), cex=1.5, box.lty=0)}
  }
}
dev.off()
...
awk '{split($1, a, ":"); if (a[1]<8) print $1 }' $dir1/snps-unrel-maf$i.use | shuf | head -n $m >$dir2/left-snps-unrel-maf$i-$nm  
awk '{split($1, a, ":"); if (a[1]>=8) print $1 }' $dir1/snps-unrel-maf$i.use | shuf | head -n $m >$dir2/right-snps-unrel-maf$i-$nm
done
...
# convert to rs system ---------------------------------------------------------
n=(0k 1k 2k 3k 4k 5k 6k)
for j in {0..6}; do
for i in {.1,.01,.001}; do
awk '(NR==FNR){a[$1]; b[$1]=$2; next}($1 in a){print b[$1], $2}' doug/ukbb.ldsc summary/ave-r2-10k-snps-mix-pop-gwas-${n[$j]}-noneuro-maf$i > summary/ave-r2-10k-snps-mix-pop-gwas-${n[$j]}-noneuro-maf$i-rs
done
done
...
awk '(NR==FNR){a[$1]; b[$1]=$2; next}($1 in a){print b[$1], $2}' doug/ukbb.ldsc gen/snps-unrel-maf.001.use > gen/snps-unrel-maf.001.use-rs
